...
const getPosts = async (req, res) => {
  const sqlQuery = `
  SELECT posts.*, users.id AS userId, users.fullName FROM posts
...
`;
  db.query(sqlQuery, (err, results) => {
    ...
    const newData = results.map((result) => {
      const { fullName, author, ...newobj } = result;
      return { ...newobj, author: { fullName, author_id: author } };
    });
...
const getPost = async (req, res) => {
  const { id } = req.params;
  const sqlQuery = `SELECT posts.*, users.id AS userId, users.fullName FROM posts
  LEFT JOIN users ON users.id = posts.author WHERE posts.id = ?`;
...
const createPost = async (req, res) => {
  ...
  db.query(sqlQuery, [data], (err, result) => {
    ...
    db.query(q, [result.insertId], (err, result) => {
      if (err) return res.status(401).json(err.message);
      return res
        .status(200)
